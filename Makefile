...
# Known U-mode programs. Each one is expected to live at
# bin/<name>/<name>.c ; adding a new binary is just adding its name here,
# nothing else needs to be edited by hand
PROGRAMS = shell yield_test
PROGRAM_OBJECTS = $(foreach p, $(PROGRAMS),bin/$(p)/$(p).o)

# Shared U-mode support code, linked into every program
COMMON_USER_OBJECTS = \
	bin/common/crt0.o \
	bin/common/syscall.o

# U-mode objects: compiled normally, then have their sections
# renamed before linking so the linker script can match them with
# a bare *(.utext)/*(.udata)/*(.urodata) wildcard. no pathglobs.
USER_OBJECTS = $(COMMON_USER_OBJECTS) $(PROGRAM_OBJECTS)
...
DEPS = $(CORE_OBJECTS:.o=.d) $(USER_OBJECTS:.o=.d)
...
clean:
	rm -f $(CORE_OBJECTS) $(USER_OBJECTS) $(USER_OBJECTS_RENAMED) $(DEPS) $(KERNEL) $(LDSCRIPT)
